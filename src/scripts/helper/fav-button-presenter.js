...
} from '../views/template/fav-button-template';
...
const FAV_BUTTON_PRESENTER = {
  ...
  async _renderButton() {
    try {
      ...
      const data = await this._favoriteResto.loadResto(id);
  ...
  _renderFavButtonTemplate() {
    ...
    favButton.addEventListener('click', async () => {
      ...
      INIT_SWAL_SUCCESS('Restoran telah ditambahkan!');
      this._renderButton();
    });
  },
  ...
  _renderUnfavButtonTemplate() {
    ...
    favButton.addEventListener('click', async () => {
      ...
      INIT_SWAL_SUCCESS('Restoran belum ditambahkan!');
      this._renderButton();
    });
  },
